    ...
    while networkClock.current_TTI < LTE_config.simulation_time_tti
        ...
        if (staticCoMP == false)
            if (mod(networkClock.current_TTI,dComp) ~= 0)  % collect learning data between dComp intervals
                ...
                [validity, inverted, model] = estimateCoMPSettings(training_data, staticCoMP, model_choice=='dnn'); % Faris: This vectorizes, cleans the measurement data, and trains ML.
            ...
            if (validity == true)
              ...
              %  if (~isnan(sum(newX(:))) && ~isinf(newX(1)) && ~isinf(newX(2)))
                ...
                if (model_choice == 'dnn')
                    ...
                    if (inverted == true)
                        disp('VERBOSE:  Inversion needed for AUC not yet implemented.');
                    end
                end
            ...
            if (fall_back == false)
                ...
                fprintf('CoMP Cluster: median vote is %0.1f.\n', median_vote);
        ...
        if (staticCoMP == true || fall_back == true)
            ...
            fprintf('CoMP Cluster: trigger is %3f.\n', trigger);
